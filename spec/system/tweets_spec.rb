...
RSpec.describe 'ツイート削除', type: :system do
  ...
  context 'ツイートの削除ができるとき' do
    it 'ログインしたユーザーは自らの投稿したツイートの削除ができる' do
      # ツイート1を投稿したユーザーでログインする
      sign_in(@tweet1.user)
...
RSpec.describe 'ツイート詳細', type: :system do
  ...
  context 'ツイートの詳細画面の表示' do
    ...
    it 'ログインしていないユーザーはツイート詳細ページに遷移できるもののコメント投稿欄が表示されない' do
      # トップページに移動する
      visit root_path
      # ツイートに詳細ボタンがあることを確認する
      expect(
      ...
      # 詳細ページに遷移する
      visit tweet_path(@tweet)
      # 詳細フォームにツイートの内容が含まれている
      expect(page).to have_content("#{@tweet.text}")
      expect(page).to have_selector ".content_post[style='background-image: url(#{@tweet.image});']"
      # フォームが存在しないことを確認する
      expect(page).to have_no_selector 'form'
      # コメントの投稿には新規登録/ログインが必要ですが表示されていることを確認する
